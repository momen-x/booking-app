...
const SingleProviderCard = async ({ id }: { id: string }) => {
  ...
  if (!response.ok) {
    return <div>Failed to fetch provider data</div>;
  }
    ...
    <div className="flex min-h-screen items-center justify-center p-4">
      <Card className="w-full max-w-lg overflow-hidden transition-all duration-300 hover:shadow-xl">
        ...
        <CardFooter className="flex flex-col gap-3 pt-2">
          ...
          <Link href={`/providers/${id}/services`} className="w-full">
            <Button className="w-full cursor-pointer">View Services →</Button>
          </Link>
        </CardFooter>
      </Card>
    </div>
